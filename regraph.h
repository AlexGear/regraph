...
enum line_styles { SOLID_LINE = PS_SOLID, DOTTED_LINE = PS_DOT, ENTER_LINE = PS_DASHDOT,
    DASHED_LINE = PS_DASH, USERBIT_LINE = PS_USERSTYLE };

enum fill_styles { EMPTY_FILL, SOLID_FILL, LINE_FILL,
				   STICK_FILL, LTSLASH_FILL, SLASH_FILL, BKSLASH_FILL,
...
enum font_names { DEFAULT_FONT, TRIPLEX_FONT, SMALL_FONT, SANS_SERIF_FONT, GOTHIC_FONT, SCRIPT_FONT,
...
enum keys { KEY_LEFT = VK_LEFT, KEY_UP = VK_UP, KEY_RIGHT = VK_RIGHT,
			KEY_DOWN = VK_DOWN, KEY_END = VK_END, KEY_HOME = VK_HOME,
			KEY_INSERT = VK_INSERT, KEY_DELETE = VK_DELETE, KEY_F1 = VK_F1,
			KEY_F2 = VK_F2, KEY_F3 = VK_F3, KEY_F4 = VK_F4, KEY_F5 = VK_F5,
			KEY_F6 = VK_F6, KEY_F7 = VK_F7, KEY_F8 = VK_F8, KEY_F9 = VK_F9,
			KEY_F10 = VK_F10, KEY_F11 = VK_F11, KEY_F12 = VK_F12 };
...
#define MAX_KQ_SIZE 100
#define MAX_MV_SIZE 100
#define CLOSE_FROM_CLOSEGRAPH 0xCC0

#define IF_WND(y) if(currentWindowIndex >= 0) { (y); }
#define IF_WND_0(y) ((currentWindowIndex >= 0) ? (y) : 0)
#define IF_WND_EX(y, n) ((currentWindowIndex >= 0) ? (y) : (n))
#define SYNCHRONIZED(mutex, action) WaitForSingleObject((mutex), INFINITE); (action); ReleaseMutex((mutex));
...
private:
    ...
    typedef void (*putFunction)(HDC dc, int x, int y, int color);
...
int initwindow(int width, int height, const char *title = "Graphic Window", int = 0, int = 0, bool = false, bool closeflag = true);
